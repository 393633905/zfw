        ...
        <form action="{{ route('admin.fangattr.update',$fangattr) }}" id="form-member-add" method="post" class="form form-horizontal">
                    ...
                    <span class="select-box">
                        <select class="select" name="pid">
                            ...
                            @foreach($data as $item)
                                <option value="{{ $item->id }}" @if($item->id==$fangattr->pid)selected @endif>{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </span>
                </div>
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-4 col-sm-4">
                    <input type="hidden" value="{{ config('up.default') }}" name="icon" id="icon"/>
                    <img src="{{ config('up.default') }}" id="pic" style="width: 60px; height: 60px">
                </div>
            </div>
            <div class="row cl">
